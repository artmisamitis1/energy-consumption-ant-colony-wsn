...
for i=1:n
    ...
    if S(i).Ecur<=0 
        ...
        for j=1:n
            ...
            if  strcmp(S(i).type ,'Direct')==0 && i~=j && dist<rc && S(i).Ecur>0
                dis(i,j)=dist;
                S(i).C=[S(i).C j];
...
ClusterHeads=[];
for i=1:n
    if strcmp(S(i).type ,'Direct')==1 || strcmp(S(i).type ,'dead')==1 ||strcmp(S(i).type ,'N')==1
         continue;
    end
    if length(S(i).C)==0
   ...
   if S(i).Clp>m 
       ...
       S(k).Ecur=S(k).Ecur - Esen;
        ...
        S(S(i).C(ind)).type='C';
        S(S(i).C(ind)).Ecur=S(S(i).C(ind)).Ecur-Esen;
   end
end
%% creat Cluster
   ...
   S(k).Ecur=S(k).Ecur-k*Esen;
    dist_tp_Bs=sqrt( (S(k).x-sink.x)^2 + (S(k).y-sink.y)^2 );
        if dist_tp_Bs<2*Rc
            ...
            for tr=1:u
                tt=ClusterHeads(tr);
                trely=(betae*S(tt).Ecur/S(tt).Emax)+(1-betae)*S(k).Cl;
...
%% send Date 
 for i=1:n
    ...
    if   S(i).Ecur>0 && strcmp(S(i).type,'C')
        S(i).Ecur=S(i).Ecur-(length(S(i).C)+1)*Esen;
    end
...
Energy2=energy;
Alive2=n-DeadSensors;

close all
save ECDC.mat Energy2 Alive2
